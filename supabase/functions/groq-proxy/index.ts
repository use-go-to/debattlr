...
interface GroqRequest {
  action: "suggest_topics" | "summarize_member" | "rank_peers" | "generate_manifesto" | "round_commentary";
...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "suggest_topics") {
      const { theme, difficulty } = payload as { theme: string; difficulty?: string };
      const difficultyMap: Record<string, string> = {
        easy: "accessibles et bien connus du grand public, avec des positions claires et des exemples concrets",
        medium: "nuancés avec plusieurs enjeux, nécessitant quelques connaissances mais restant compréhensibles",
        hard: "complexes, controversés et philosophiquement profonds, nécessitant une vraie réflexion critique",
      };
      const diffDesc = difficultyMap[difficulty || "medium"];
...
Propose exactement 3 sujets de débat ${diffDesc}, formulés comme des questions ouvertes stimulantes.
...
En tenant compte des critères et des votes des pairs, donne un classement et réponds UNIQUEMENT avec :
{
  ...
  "overall_analysis": "analyse globale du débat en 3-4 phrases"
    ...
    // ── 4. Round commentary (TV journalist style) ────────────────────────────
    else if (action === "round_commentary") {
      const { topic, round, turns } = payload as {
        topic: string;
        round: number;
        turns: Array<{ name: string; content: string }>;
      };
      systemPrompt = "Tu es un journaliste-commentateur de débat télévisé, style grand débat politique français. Ton ton est vif, incisif, légèrement dramatique mais toujours juste.";
      userPrompt = `Débat en cours sur : "${topic}"
Fin du Round ${round}.

Arguments échangés ce round :
${turns.map((t) => `${t.name} : "${t.content}"`).join("\n")}

En tant que commentateur TV, fais un bref résumé du round (4-6 phrases max) :
- Qui a dominé ce round et pourquoi
- La tension ou les points de friction
- Ce qu'on attend pour la suite
Ton direct, percutant, sans JSON, sans titre.`;
    }

    // ── 5. Generate final manifesto ───────────────────────────────────────────
...
Écris directement le texte du manifeste, sans JSON, sans titre markdown.`;
    ...
    if (action === "generate_manifesto" || action === "round_commentary") {
